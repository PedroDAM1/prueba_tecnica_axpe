...
class UserRepositoryImpl @Inject constructor (
    ...
    override fun getUserByUuid(uuid: String): Flow<DataResponse<UserModel>> = flow {
        ...
        val user = userCache.getUsers().find {
    ...
    override fun searchByName(name: String): Flow<DataResponse<List<UserModel>>> = flow {
        val filtered = userCache.getUsers().filter {
    ...
    private suspend fun mapUsers(responseApi : DataResponse<ApiResponseInfo>) : DataResponse<List<UserModel>>{
        return when(responseApi){
            is DataResponse.Success -> {
                ...
                val domainList = responseApi.data.results.mapNotNull { userApi ->
                    ...
                    if(userCache.getUsers().contains(userDomain))null
                    else {
                        //Guardamos el usuario en la cache
                        userDomain
                    }
                }
                userCache.saveUserData(domainList)
